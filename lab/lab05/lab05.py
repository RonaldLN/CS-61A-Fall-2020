...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    refer = make_city("refer", lat, lon)
    return get_name(city_a) if distance(refer, city_a) < distance(refer, city_b) else get_name(city_b)
...
def add_trees(t1, t2):
    ...
    # common_branches_num = min(len(branches(t1)), len(branches(t2)))
    # return tree(label(t1) + label(t2), [add_trees(branches(t1)[i], branches(t2)[i]) for i in range(common_branches_num)] + branches(t1)[common_branches_num:] + branches(t2)[common_branches_num:])

    # result_label = label(t1) + label(t2)
    # result_branches = []
    # i = 0
    # while i < min(len(branches(t1)), len(branches(t2))):
    #     result_branches += [add_trees(branches(t1)[i], branches(t2)[i])]
    #     i += 1
