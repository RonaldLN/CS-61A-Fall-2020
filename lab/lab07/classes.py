...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        opponent.hand = opponent.hand[3:] + [opponent.deck.draw() for _ in range(3)]
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for i in range(orig_opponent_deck_length, 0, -1):
            if opponent.deck.cards[i - 1].attack == other_card.attack or opponent.deck.cards[i - 1].defense == other_card.defense:
                opponent.deck.cards.pop(i - 1)
