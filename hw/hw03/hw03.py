...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y) < 0 or lower_bound(y) > 0, "the interval spans zero"
...
def check_par():
    ...
    r2 = interval(11, 13) # Replace this line!
    return r1, r2
...
def quadratic(x, a, b, c):
    ...
    upper = upper_bound(x)
    if a == 0:
        return interval(f(lower), f(upper)) if b > 0 else interval(f(upper), f(lower))
    extreme_point = -b / (2 * a)
    if extreme_point > lower_bound(x) and extreme_point < upper_bound(x):
        if a > 0:
            return interval(f(extreme_point), max(f(lower), f(upper)))
        else:
            return interval(min(f(lower), f(upper)), f(extreme_point))
    else:
        return interval(f(lower), f(upper)) if f(lower) < f(upper) else interval(f(upper), f(lower))
