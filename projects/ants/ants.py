...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        #     bee.reduce_armor(amount if self.armor > 0 else amount + self.damage)
        # if self.armor <= 0:
        #     # for bee in self.place.bees:
        ...
        # for bee in self.place.bees:
        #     bee.reduce_armor(amount if self.armor - amount > 0 else amount + self.damage)
        # Ant.reduce_armor(self, amount)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        # dist = 1
        # result = {}
        # while place is not self.place:
        #     if place.ant:
        #         result[place.ant] = -dist
        #     result += {bee: -dist for bee in place.bees}
        #     place = place.exit
        #     dist += 1
        # if self.place.ant is not self:
        #     result[self.place.ant] = -dist
        # return {insect: result[insect] + dist for insect in result}

        result = {}
        if self.place.ant is not self:
            result[self.place.ant] = 0
        for bee in self.place.bees:
            result[bee] = 0
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        # return 0
        return max(self.damage - 0.2 * distance - 0.05 * self.insects_shot, 0)
        # END Problem Optional 5
